...
int main() {
	...
	builder.Connect(generator->get_output_port(), controller->GetInputPort("x_tr"));
	builder.Connect(plant->GetOutputPort("x"), controller->GetInputPort("x"));
	builder.Connect(controller->GetOutputPort("u"), plant->GetInputPort("u"));
